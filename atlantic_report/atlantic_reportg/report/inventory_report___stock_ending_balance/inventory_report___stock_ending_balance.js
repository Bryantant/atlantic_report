frappe.query_reports["Inventory Report - Stock Ending Balance"] = {
onload(report) {
    ...
    formatter(value, row, column, data, default_formatter) {
        ...
        if (data && data.is_total_row) {
            style += "font-weight:bold;background-color:#fafafa;";
        }
        ...
        // =============================
        // FORMAT CURRENCY 2 DECIMAL
        // =============================
        const numeric_matches = [...html.matchAll(/([\d,]+\.\d+|[\d,]+)/g)];

        if (numeric_matches.length > 0) {
            numeric_matches.forEach(m => {
                let raw = m[0].replace(/,/g, "");
                let num = flt(raw);
                let formatted = format_number(num, null, 2);

                // Replace only the numeric part, keep HTML structure
                html = html.replace(m[0], formatted);
            });
        }
